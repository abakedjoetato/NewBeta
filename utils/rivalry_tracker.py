...
class RivalryTracker:
    ...
    async def update_rivalry_on_kill(db, kill_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Skip update if missing critical information
            if not killer_id or not victim_id or not server_id:
    ...
    async def _update_prey_data(db, server_id: str, killer_id: str, victim_id: str) -> bool:
        ...
        try:
            ...
            if victim_id not in killer_doc['prey']:
                killer_doc['prey'][victim_id] = {
                    'kills': 1,
                    'player_id': victim_id,
                    'player_name': victim_doc['name'] if victim_doc else "Unknown Player",
            ...
            if victim_doc:
                ...
                reverse_kill_count = nemesis_data.get('kills', 0) if nemesis_data else 0
    ...
    async def _update_nemesis_data(db, server_id: str, victim_id: str, killer_id: str) -> bool:
        ...
        try:
            ...
            if killer_id not in victim_doc['nemesis']:
                victim_doc['nemesis'][killer_id] = {
                    'kills': 1,
                    'player_id': killer_id,
                    'player_name': killer_doc['name'] if killer_doc else "Unknown Player",
                    'last_kill': datetime.utcnow()
                }
            else:
            ...
            if killer_doc:
                ...
                reverse_kill_count = prey_data.get('kills', 0) if prey_data else 0

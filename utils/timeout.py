...
async def with_timeout(
    ...
    """
    Execute a coroutine with a timeout, returning a default value if the timeout is exceeded.
    
    Args:
        coroutine: The coroutine to execute
        timeout: Timeout in seconds (default: 2.0)
        default_value: Value to return if timeout is exceeded (default: None)
        operation_name: Name of the operation for logging
        log_error: Whether to log an error message on timeout
        
    Returns:
        The result of the coroutine, or default_value if timeout is exceeded
    """
    try:
